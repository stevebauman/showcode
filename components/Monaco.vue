...
import { uniq, get, range, union, difference } from 'lodash-es';
import useApplicationStore from '@/composables/useApplicationStore';
import usePreferencesStore from '@/composables/usePreferencesStore';

// Import all monaco themes using Vite's glob import
const themeModules = import.meta.glob('/node_modules/monaco-themes/themes/*.json', { eager: true });
...
const { language, tabSize, modelValue: value, width, height, added, removed, focused } = toRefs(props);
...
function makeDecoration(rangeObj, type) {
    return {
        options: {
            stickiness: 1,
...
Object.keys(themes).forEach((theme) => {
    ...
    const modulePath = `/node_modules/monaco-themes/themes/${filename}.json`;
    const themeData = themeModules[modulePath]?.default || themeModules[modulePath];
...
const makeHighlightCallback = (className, refValue) => () => {
    let selected = selectionToRange(editor.value.getSelection());

    const alreadySelected = unref(refValue).filter((line) => selected.includes(line));
    ...
    if (difference(selected, alreadySelected).length === 0) {
        selected = unref(refValue).filter((line) => !alreadySelected.includes(line));
    } else {
        selected = union(unref(refValue), selected);
    }

    emit(`update:${className}`, selected);
};

const makeDecosCallback = (className, refValue) => (lines) => {
    refValue.value = editor.value.deltaDecorations(
        refValue.value,
        lines.map((line) =>
            makeDecoration(
                {
